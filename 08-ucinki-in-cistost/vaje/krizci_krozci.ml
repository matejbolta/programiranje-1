...
let place_token player (grid : grid) (row_i, col_i) : state =
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      let winner_message = 
        match result with
        | Tie -> "\nNihče ni zmagal igre!"
        | Winner p -> "\nZmagal je igralec " ^ show_player p ^ "!"
      in
...
let rec choose kind = (* kind = "vrstico" || kind = "stolpec") *)
